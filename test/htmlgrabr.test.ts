...
describe("Smoke test", () => {
  ...
  it("simple cleanup test case", async () => {
    const grabber = new HTMLGrabr({ debug: true })
    expect(grabber.config.debug).toBeTruthy()
    const page = await grabber.grab('<html><head><title>Test</title></head><body><p>Hello World!</p></body></html>')
    expect(page).not.toBeNull()
    expect(page.title).toEqual('Test')
    expect(page.html).toEqual('<p>Hello World!</p>')
    expect(page.text).toEqual('Hello World!')
  })
  ...
  it("iterate over all cleanup test cases", async () => {
    ...
    for (const filename of files) {
      ...
      let match = filename.match(/^(.+)_(expected|input|meta)\.(html|json)$/)
    ...
    // Run tests
    const grabber = new HTMLGrabr({ debug: false })
